/**
 * SolarisSwahili v2.0
 * ساعة التوقيت السواحلي الديناميكية — التوقيت التكيفي المعياري
 */
...
const App = (() => {
    ...
    const API = {
        ...
        fetchSolar: async (lat, lng) => {
            ...
            // دالة لسحب طول النهار الرسمي من الـ API بالثواني وتحويله لمللي ثانية
            const parseDur = str => {
                if (!str) return 0;
    ...
    const loadCity = async key => {
        ...
        try {
            ...
            /* ── شريط النهار والليل (معتمد على المصدر الرسمي) ── */
            const dayL   = solar.dayLengthMs; // من الـ API مباشرة
            const nightL = Math.max(0, (24 * 3600000) - dayL); // اليوم دائماً 24 ساعة
